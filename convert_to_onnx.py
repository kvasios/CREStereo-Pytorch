...
if __name__ == '__main__':
	...
	model_path = models_dir / "crestereo_eth3d.pth"
	...
	torch.onnx.export(model,               
	                  (t1, t2, flow_init),
	                  str(models_dir / "crestereo.onnx"),   # where to save the model (can be a file or file-like object)
	...
	torch.onnx.export(model,               
	                  (t1_half, t2_half),
	                  str(models_dir / "crestereo_without_flow.onnx"),   # where to save the model (can be a file or file-like object)
	                  export_params=True,        # store the trained parameter weights inside the model file
	                  opset_version=12,          # the ONNX version to export the model to
	                  do_constant_folding=True,  # whether to execute constant folding for optimization
